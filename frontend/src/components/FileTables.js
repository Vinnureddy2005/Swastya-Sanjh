...
const FileTables = ({ files, onView, onDelete }) => {
  const handleViewClick = (fileId) => {
    console.log('Clicked on VIEW button with fileId:', fileId);
    onView(fileId);
  };
          ...
          {files.map((file, index) => (
            <tr key={index}>
              ...
              <td className="border-y border-r  py-2 pl-16  flex justify-end items-center border-black">
                ...
                <button className="bg-red-600 hover:bg-red-800 text-white py-3 px-8 rounded mr-2" onClick={() => handleDeleteClick(file.id)}>
